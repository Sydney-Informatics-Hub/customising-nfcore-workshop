...
Unless you are actively developing workflow code, you do not need to clone the workflow code from GitHub and can use Nextflow’s built-in functionality to `pull` amd store the workflows for you. As shown in the [Introduction to Nextflow](1.1_nextflow.md), The Nextflow `pull` command to download and cache nf-core workflows from the [nf-core GitHub](https://github.com/nf-core/) repository:
