...
As we explored in [Lesson 1.4.2](1.4_users.html#nf-core-download) we can fetch the workflow code base with the following command:
...
As we learned in [lesson 1.3.3](1.3_configure.html#viewing-parameters), all nf-core pipelines have a unique set of pipeline-specific parameters that can be used in conjunction with Nextflow parameters to configure the workflow. Generally, nf-core pipelines can be customised at a few different levels:
...
2. Provided a choice of multiple workflows different methods and specified which is the default
3. Provided a choice of tool for some processes
...
1. Provided pipeline-specific parameters (i.e. `--param_name`)
2. Nextflow parameters (i.e. `-profile`)
3. Additional process-specific parameters using the `ext.args` variable (See lesson 2.5)
...
The inclusion of `ext.args` is currently standard practice for all DSL2 [nf-core modules](https://nf-co.re/developers/modules) where additional parameters may be required to run a process. However, this may not be implemented for all processes. Depending on the pipeline, these process modules may not have defined the `ext.args` variable. 
...
1. Providing our own reference fasta, index files, and gtf using [file-specific parameters](https://nf-co.re/rnaseq/3.11.1/parameters#reference-genome-options) instead of using the Illumina AWS iGenomes database parameter `--genomes`
2. Using Nextflow's `-profile` parameter to specify that we will be running the Singularity profile as [specified in `nextflow.config`](https://github.com/nf-core/rnaseq/blob/3.11.1/nextflow.config#L144) instead of Docker
