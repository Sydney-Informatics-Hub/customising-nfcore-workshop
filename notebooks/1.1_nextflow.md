...
The complete URL could be used:

```bash
nextflow pull http://github.com/foo/bar
```

Alternatively, the `clone` command can be used to download a pipeline into a local directory of your choice.

When you `run` a pipeline with Nextflow, it will look for a file with the pipeline name you’ve specified. If that file does not exist, it will look for a public repository with the same name on GitHub (unless otherwise specified). If it is found, the repository is automatically `pull` the pipeline to your global cache and execute it.
