...
You can control the Nextflow runtime and the underlying Java virtual machine using [environment variables](https://www.nextflow.io/docs/latest/config.html#environment-variables). These variables can be exported before running a Nextflow workflow and will be interpreted as environment variable by Nextflow.
...
Over time you might want to remove or update your stored workflows. Nextflow has functionality to You can also manage what workflows you have stored locally using Nextflow.
...
If you want to remove a workflow from your cache you can remove it using the `drop` command:
