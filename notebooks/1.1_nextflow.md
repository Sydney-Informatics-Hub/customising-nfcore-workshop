...
If your local version of a workflow is not the latest you be shown a warning and will be required to use a revision flag when executing the workflow. You can update a pipeline with the Nextflow `pull` command.
...
- The Nextflow `pull`, `run`, and `clone` commands can be used to download and store Nextflow workflows.
