...
- Learn fundamental Nextflow options and commands.
...
If you do not already have Nextflow available, it can be installed using a few easy steps:
...
It is worth noting that when you `run` a workflow, it will look for a local file with the workflow name you’ve specified. If that file does not exist, it will look for a public repository with the same name on GitHub (unless otherwise specified). If it is found, Nextflow will automatically `pull` the workflow to your global cache and execute it.

::: callout-tip

### **Challenge**{.unlisted}

Run the `hello` workflow directly from `nextflow-io` [GitHub](https://github.com/nextflow-io/hello) repository.
:::
...
Run the `hello` workflow directly from `nextflow-io` GitHub using the `v1.1` revision tag.
:::
...
The warning shown above is expected. The warning is shown because the `v1.1` workflow revision was written using an older version of Nextflow that uses the depreciated `echo` method.
...
This command has multiple options to facilitate the queries and is especially useful while debugging a pipeline and while inspecting pipeline execution metadata. You can view all of the possible `log` options with `-h` flag:
...
2. [`FASTQC`](https://github.com/Sydney-Informatics-Hub/nf-core-demo/blob/master/modules/nf-core/fastqc/main.nf)
   - Executes the `fastqc` command using the `.fastq.gz` files from the [sample sheet](https://raw.githubusercontent.com/nf-core/test-datasets/viralrecon/samplesheet/samplesheet_test_illumina_amplicon.csv).
3. [`MULTIQC`](https://github.com/Sydney-Informatics-Hub/nf-core-demo/blob/master/modules/nf-core/multiqc/main.nf)
   - Executes `multiqc` using the `fastqc` reports.

The [`Sydney-Informatics-Hub/nf-core-demo`](https://github.com/Sydney-Informatics-Hub/nf-core-demo/tree/master) workflow is more advanced than the `nextflow-io/hello` workflow that has been shown previously. It uses real data and bioinformatics software and requires additional configuration to run successfully. To run this example you will need to include two profiles in your execution command. Profiles are sets of configuration options that can be accessed by Nextflow and will be explained in greater detail during [configuring nf-core workflows](1.3_configure.md#1.3.5.-default-configuration-files) section of the workshop.
...
Running this workflow will create a `work` directory and a `my_results` directory with selected results files.

In this example, the hexadecimal numbers, such as `ff/21abfa`, identify the unique task execution. These numbers are also the prefix of the `work` directories where each task is executed. You can inspect the files produced by a task by looking inside the `work` directory and using these numbers to find the task-specific execution path:
...
If you were to look inside the `work` directory a `FASTQC` task, you will find the files that were staged and created when this task was executed. You will also hidden `.command` files. You will also see that Nextflow created symlinks for your files rather than creating a new copy of each file in the task directories:

![](../figs/1.1_lsla.excalidraw.png)

The `FASTQC` process runs four times, executing in four different work directories for each set of inputs. Therefore, in the previous example the work directory [1a/3c54ed] represents just one of the four directories that were processed. To print all the relevant paths to the screen, use the `-ansi-log` option can be used:

```bash
nextflow run Sydney-Informatics-Hub/nf-core-demo -profile test,singularity -ansi-log false
```

It's very likely you will execute a workflow multiple times as you are finding the parameters that best suit your data. As such, you can save a lot of spaces (and time) by **resuming** a workflow from the last step that was completed successfully and/or unmodified after your changes. By adding the `-resume` option to your `run` command you can use the cache rather than re-running tasks:

```bash
nextflow run Sydney-Informatics-Hub/nf-core-demo -profile test,singularity -resume
```

If you run the Sydney-Informatics-Hub/nf-core-demo workflow again without any changes you will see that the workflow will use the cache rather than executing tasks again:
...
Invalidate the cache by touching a `.fastq.gz` file in a `FASTQC` task work directory. Run the workflow again with the `-resume` option to show that the cache has been invalidated.
...
Use the task ID shown for the `FASTQC` process and use it to find and `touch` a the `sample1_R1.fastq.gz` file. Your path will likely be different to the path shown here:
...
You should see that the cache of 2 of 4 tasks for the `FASTQC` process and the `MULTIQC` process were invalid.
...
Your work directory can get very big very quickly (especially if you are using full sized datasets). It is beneficial to `clean` your the cache regularly. Rather than removing the `work` folder with all of it's contents, the Nextflow `clean` function allows you to selectively remove data associated with specific runs.
