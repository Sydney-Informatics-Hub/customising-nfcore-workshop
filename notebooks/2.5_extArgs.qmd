...
nf-core modules provide all optional non-file arguments as a string using the [`ext` directive](https://www.nextflow.io/docs/latest/process.html#ext) via the `$task.ext.args` variable. On the left of the diagram below, an example of the standard `main.nf` format of a process stored in the `modules/` directory is shown. Above the script block, the `$args` variable is defined. Inside the script block the `$args` variable is applied to the process command.
...
To practice this, we are going to pass an optional flag to the `TRIMGALORE` process. It is up to you to decipher what customisations are required to make your analysis suitable for your data and research questions. In order to understand what customisations you may want to specify as extra arguments, we recommend you: 
...
3. View the nf-core/rnaseq pipeline `TRIMGALORE` [`main.nf`](https://github.com/nf-core/rnaseq/blob/master/modules/nf-core/trimgalore/main.nf) file
...
3. What optional flags are applied to the `TRIMGALORE` process? 
:::
...
To parse that to the `TRIMGALORE` process, we need to specify this in a custom config. This time, our `TRIMGALORE` custom configuration file will restrict the usage of the parameter to the only place it applies, the `TRIMGALORE` process. We can do this using the Nextflow `process {}` scope and `withName:` selector. 
