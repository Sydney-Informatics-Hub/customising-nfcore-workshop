...
1. `--quality` or `-q`
2. No, but there is an `--extra_trimgalore_args` parameter
3. `--cores` and `--gzip` 
:::

The `--extra_trimgalore_args` parameter isn't explained very well, so we will follow the Trim Galore user guide, to apply the `--quality` flag with a Phred score of 40:
