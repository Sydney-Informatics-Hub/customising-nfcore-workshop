...
[![](../figs/1.2_parametersweb.png)](https://nf-co.re/rnaseq/3.11.1/parameters)
...
The extended execution path is built from the workflows, subworkflows, and modules used to execute the process. In the example above, the nf-core [`TRIMGALORE`](https://github.com/nf-core/rnaseq/blob/master/modules/nf-core/trimgalore/main.nf) module, was called by the [`FASTQ_FASTQC_UMITOOLS_TRIMGALORE`](https://github.com/nf-core/rnaseq/blob/master/subworkflows/nf-core/fastq_fastqc_umitools_trimgalore/main.nf) subworkflow, which was called by the [`RNASEQ`](https://github.com/nf-core/rnaseq/blob/master/workflows/rnaseq.nf) workflow, which was called by the `NFCORE_RNASEQ` workflow in the [`main.nf`](https://github.com/nf-core/rnaseq/blob/master/main.nf) file in the `nf-core/rnaseq` repository.
...
Multiple scopes can be included in the same `.config` file. A full list of scopes and is described in detail [here](https://www.nextflow.io/docs/latest/config.html#config-scopes).
