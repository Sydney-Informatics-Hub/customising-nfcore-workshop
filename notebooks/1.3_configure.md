...
You can be clever with the what you configure in each of these files. For example, you might consider configuring parameters that are unique to you as a user (such as your name and email address) in your home directory as this can be applied to every execution.
...
You can include multiple custom `.config` files at the same time by separating them using a `,`.
