...
#### **2.4.2. Customised resource tracing**{.unlisted}

Remember we can use the `nextflow log` command to show information about previous pipeline executions. We can customise which fields to print using the [`-fields` flag (-f)](https://www.nextflow.io/docs/latest/tracing.html#customizing-fields). This flag accepts options used by the [trace report](https://www.nextflow.io/docs/latest/tracing.html#trace-fields). 
...
Make sure both YAML files are saved, then rerun the workflow:
