...
There is a [newer version of Trim Galore](https://github.com/FelixKrueger/TrimGalore/releases/tag/0.6.10) available: v.0.6.10. 
...
Recall that we have been accessing our training materials from [**CernVM-FS**](https://support.pawsey.org.au/documentation/pages/viewpage.action?pageId=92832676#NimbusforBioinformatics-1.BiocontainersandReferenceGenomedata ):
  ...
  - CernVM-FS is a read-only filesystem which provides shared access to common bioinformatics reference datasets as well as Singularity containers of everything stored in [Biocontainers](https://biocontainers.pro/)
  - CernVM-FS comes pre-mounted on Pawsey Nimbus VMs using the [**Pawsey Bio - Ubuntu 22.04 - 2023-03 image**](https://support.pawsey.org.au/documentation/display/US/Nimbus+for+Bioinformatics#) image
...
Alternatively, we could specify an alternate remote source, such as [quay.io](https://quay.io/organization/biocontainer), or point to a local image. 

<br/>

::: callout-tip
### **Poll question** :hand:
:question: For maintaining portability and reproducibility, what is the best way to specify our custom Trim Galore container when we execute the nf-core/rnaseq pipeline? 
...
b) Within a separate custom config file named `custom-tools.config`. We would then apply it at `-c` along with our institutional config whenever it was required. 
...
&#x27A4; Then copy the Trim Galore module execution path from your terminal inside the `withName` block:
...
:hourglass: <span style="color:#8b0000;">*Reminder to check the `Lesson-2.4.1` run!*</span>
<br/><br/>

Let's make a portable configuration file for resource tracing. Because this config is not infrastructure or pipeline specific, we can share it with collaborators and use it across multiple pipelines :handshake: 
...
&#x27A4;  Save the config and resume your previous run, adding `custom-trace.config` and setting `outdir` to `Lesson-2.4.2`:
...
&#x27A4; View the html file `Lesson-2.4.3/multiqc/star_salmon/multiqc_report.html` and compare it to the report generated by a previous run:
...
:brain: Try to write the config without showing the solution!
...
&#x27A4;  Save `custom-args.config` then resume the run, specifying our latest custom config file at `-c` and set `outdir` param to `Lesson-2.4.4`:
