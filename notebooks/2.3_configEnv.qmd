...
In order to do this, we need to use the `process` scope. Nextflow has a number of different [`scopes`](https://www.nextflow.io/docs/latest/config.html) that can be included in configuration files, for example the `params` scope you will recall from [lesson 1.3](1.3_configure.html).

Within the `process` scope, we can configure resources and additional arguments for workflow processes.
<br/>
