...
The `base.config` file applies default compute resource settings to be used by the processes in the nf-core workflow. It uses process labels to enable different sets of resources to be applied to groups of processes that require similar compute. These labels are specified within the main.nf file for a process.  

The `nextflow.config` file is more workflow-specific, and sets the defaults for the workflow parameters, as well as defines profiles to change the default software access from $PATH to the specified access method. 


Thinking of the compute resource and software configuration settings we have applied to our rnaseq runs so far (`-profile singularity`, `--max_cpus`, `--max_memory`), if we were to run the nf-core/rnaseq workflow without these custom configurations, do you think the run would complete successfully? If not, why?
...
The parameters --max_cpus, --max_memory and --max_time are applied as a cap for a workflow, to ensure that the processes do not attempt to use more resources than you have available on your platform. Thus, they are critical customisations that should be checked and where required, adjusted, depending on where you are running the pipeline. 

Within `base.config`, the `check_max()` function over-rides the process resources *if the max setting is lower than the default for the group of processes with that label* 
