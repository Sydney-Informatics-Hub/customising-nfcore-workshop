...
![](../figs/2.3_config_priority_order_no_text_overlay.png)
<br/>
...
> <span style="color:#000080;"> Setting these values to the maximum or near-maximum on your machine/node is an important customisation to prevent uneccessary failed runs.</span>  
...
:bulb: <span style="color:#8b0000;"> This is all possible with custom configuration files!</span>
<br/><br/>
...
There is a [repository of institutional configs](https://github.com/nf-core/configs/tree/master) for nf-core pipelines. These have been contributed to by the community. 
...
If you appled `--max_cpus 16` to the nf-core rnaseq workflow, the STAR_ALIGN module would still only utilise 12 CPUs, as this module (as we learned in 2.3.1) has the label `process_high` which sets CPUs to 12.
...
::: {.callout-note}
The following exercise is trivial given the limitations of our VMs. Consider how this approach can be really powerful when working on a high performance cluster, where modules can be directed to specific queues with the `queue` process directive. See [Tips and tricks](../tips_tricks.html#Send processes to a job scheduler on HPC) for an example HPC institutional config. 
:::
<br/>
...
>:eyes: Notice that the **Max job request options** are no longer listed on the run log printed to screen
...
- Recall from [Lesson 1.3.5 'Default configuration files'](1.3_configure.html#default-configuration-files) the following tips: 
